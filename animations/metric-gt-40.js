(() => {
  const TAG_NAME = 'atlas-metric-gt-40';
  if (customElements.get(TAG_NAME)) return;

  class AtlasMetricGt40 extends HTMLElement {
    constructor() {
      super();
      ...
      this.shadowRoot.innerHTML = `
        ...
        <div class="atlas-metric" role="img" aria-label="0 percent text count animation with transparent background">
          <div class="label" aria-live="polite">
            <span class="prefix"></span><span class="number">0</span><span class="suffix">%</span>
          </div>
        </div>
      `;

      this.prefixNode = this.shadowRoot.querySelector('.prefix');
    ...
    _formatValue(value) { return String(Math.round(value)); }

    _setValue(rawValue, completed) {
      const value = this._clamp(rawValue, 0, this.target);
      const display = this._formatValue(value);
      const prefix = completed ? '>' : '';
      if (this.prefixNode) this.prefixNode.textContent = prefix;
      if (this.numberNode) this.numberNode.textContent = display;
      if (this.wrapper) this.wrapper.setAttribute('aria-label', `${prefix}${display} percent text count animation with transparent background`);
    }

    _animate() {
      const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if (reduceMotion) { this._setValue(this.target, true); return; }
      ...
      const step = (timestamp) => {
        ...
        if (progress < 1) {
          this._setValue(this.target * this._easeOutQuint(progress), false);
          this._frame = requestAnimationFrame(step);
        } else {
          this._setValue(this.target, true);
        }
      };
      this._frame = requestAnimationFrame(step);
    }
  ...
  customElements.define(TAG_NAME, AtlasMetricGt40);
})();
