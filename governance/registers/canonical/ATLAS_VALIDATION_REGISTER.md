...
The source-reconciled MVA/accounting memorandum v0.8.0 controls the restricted Norway-only monthly scope and supersedes v0.7.0 for that scope. Source-answerable specifications are complete, while payment-provider onboarding, live Conta configuration, generated sales documents/credit notes, representative imported-service postings, sandbox reconciliation and final acceptance remain open. Atlas AI and AI-Memory-Vault root-folder drift identified in this session has been corrected. Consumer checkout remains `NO_GO`; `publicAllowed=false`, `purchaseAllowed=false`, checkout disabled and Wix Pricing Plans count 0 remain controlling.
